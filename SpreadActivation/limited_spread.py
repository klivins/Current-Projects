...
#Function to infect the desired number of "students" based on proximity to the
#"infector"/"coach" node, and then number of children.  
def spread_limit(desired_activation_number, activated_nodes, nodes):
    
    #Find the largest distance that any node is away from the infection node,
    #and make it available as a counter
    largest_dist = max(nodes, key = lambda node: node.distance).distance

    #For each value in that max distance, infect nodes at each level farther away
    ...
    for dist in range(largest_dist):
        ...
        #If the number of nodes in the level is less than the desired infection
        #number, then infect the whole level
        if (len(considering)+len(activated_nodes)) < desired_activation_number:
            for node in considering:
                node.activated= True
                activated_nodes.append(node)
        
        #If infecting the number of nodes in the level would infect more nodes
        #than desired, search all the nodes for the fewest node connections in the
        #next farthest level. Infect that node, then repeat until the desired
        #number of infected nodes is reached.
        else:
            
            #Find the number of nodes that still need to be activated
            available = desired_activation_number-len(activated_nodes)
    
            #While there are some available, consider nodes for infection.
            while available >= 0:
                ...
                #Infect the current lowest node, add it to the activated list,
...
#Initiate a variable to track whether the network has reached desired infection
#Change this if you want to a greater or smaller number of people
desired_activation_number = 20

#Choose the node that will be the gensis of the infection
start_node = choice(nodes)
start_node.activated = True
activated_nodes.append(start_node)

#Check if the network is smaller than the desired infection number. If not,
#continue the infection process. If so, infect everything and tell the user
#that the size of the network is insufficient. 
if desired_activation_number < len(nodes):
    #Run the function to determine node distances from the "instructor node"
    describe_network(network, start_node)

    #Run the function to infect the desired number of nodes.
...
#Print the final infection outcome
for node in nodes:
